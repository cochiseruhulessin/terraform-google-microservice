...
locals {
  ...
  variables = merge(
    ...
    # compatibility.
    (var.frontend && length(local.backend_paths) == 0) ? {"ASGI_ROOT_PATH"="/api"} : {},
    (var.frontend && length(local.backend_paths) == 0) ? {"HTTP_MOUNT_PATH"="/api"} : {},
    ...
    metadata {
      annotations = merge({
        ...
        "run.googleapis.com/secrets": (length(var.secrets) > 0) ? join(",",
          ...
          ], (length(var.sql_databases) > 0) ? ["${local.sql_secret}:projects/${data.google_project.svc.number}/secrets/${local.sql_secret}"] : [])
        ) : null
      },
